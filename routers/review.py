...
def submit_review(
    ...
    if user_id != current_user.id:
        raise HTTPException(
            status_code=403,
            detail="You are not allowed to submit a review for another user.",
        )
    ...
    if not booking.user_id == user_id == current_user.id:
        raise HTTPException(
    ...
    if booking.status != "confirmed":
        raise HTTPException(
            status_code=400, detail="You can only review confirmed bookings."
        )
...
def filter_reviews(
    ...
    start_date: Optional[date] = Query(
        None, description="Filter reviews from this date"
    ),
    end_date: Optional[date] = Query(
        None, description="Filter reviews until this date"
    ),
):
    ...
    if not current_user:
        if user_id:
            raise HTTPException(
                status_code=403, detail="Login required to filter by user_id."
            )
        status = IsReviewStatus.confirmed
    ...
    # 👥 Logged-in normal user
        ...
        if user_id is not None and user_id != current_user.id:
            raise HTTPException(
                status_code=403, detail="You cannot filter reviews for another user."
            )

        if not user_id:
            user_id = current_user.id  # allow personal review history

        if booking_id and not db_review.booking_belongs_to_user(
            db, current_user.id, booking_id
        ):
            raise HTTPException(
                status_code=403,
                detail=f"Booking ID {booking_id} does not belong to you.",
